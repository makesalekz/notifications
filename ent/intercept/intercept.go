...
// The LastReadNotificationFunc type is an adapter to allow the use of ordinary function as a Querier.
type LastReadNotificationFunc func(context.Context, *ent.LastReadNotificationQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f LastReadNotificationFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.LastReadNotificationQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.LastReadNotificationQuery", q)
}

// The TraverseLastReadNotification type is an adapter to allow the use of ordinary function as Traverser.
type TraverseLastReadNotification func(context.Context, *ent.LastReadNotificationQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseLastReadNotification) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseLastReadNotification) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.LastReadNotificationQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.LastReadNotificationQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.LastReadNotificationQuery:
